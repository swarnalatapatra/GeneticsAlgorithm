...
ELITIST=0.18;    % percentage of the elite population
...
repr = ['Path     '; 'Adjacency'];
mut = ['Inversion'; 'Insertion'];
select = ['Sus       '; 'Tournament'; 'RWS       '];
...
selectionpopuptxt = uicontrol(ph,'Style','text','String','Selection','Position',[350 45 130 20]);
selectionpopup = uicontrol(ph,'Style','popupmenu','String',select,'Value',REP, 'Position',[450 50 130 20],'Callback',@selectionpopup_Callback);


ncitiesslidertxt = uicontrol(ph,'Style','text','String','# Cities','Position',[0 230 130 20]);
    

representationpopuptxt = uicontrol(ph,'Style','text','String','Representation','Position',[120 45 130 20]);
representationpopup = uicontrol(ph,'Style','popupmenu','String',repr,'Value',REP, 'Position',[225 50 130 20],'Callback',@reppopup_Callback);


mutationpopuptxt = uicontrol(ph,'Style','text','String','Mutation','Position',[120 5 130 20]);
mutationpopup = uicontrol(ph,'Style','popupmenu','String',mut,'Value',MUT, 'Position',[225 10 130 20],'Callback',@mutationpopup_Callback);
...
crossover = uicontrol(ph,'Style','popupmenu', 'String',{'xalt_edges','edges_recombine','partially_mapped_crossover'}, 'Value',1,'Position',[10 50 130 20],'Callback',@crossover_Callback);
...
%         y=data(:,2)/max([data(:,1);data(:,2)]);
    ...
    function elitslider_Callback(hObject,eventdata)
        fslider_value = get(hObject,'Value');
        slider_value = round(fslider_value);
        set(FhObject,'Value',slider_value);
    ...
    function runbutton_Callback(hObject,eventdata)
        ...
        run_ga(x, y, NIND, MAXGEN, NVAR, ELITIST, STOP_PERCENTAGE, PR_CROSS, PR_MUT, CROSSOVER, LOCALLOOP, REP,MUT,SEL, ah1, ah2, ah3,1);
        end_run();
    end
    ...
    function end_run()
     ...
     function reppopup_Callback(hObject, eventdata)
        REP = get(hObject, 'Value');
     end
    function mutationpopup_Callback(hObject, eventdata)
        MUT = get(hObject, 'Value');
    end
    function selectionpopup_Callback(hObject, eventdata)
